...
def send_msg(msg, client):
    ...
    client.sendto(send_len, ADDR)
    len_sent = 0
    while len_sent < msg_len:
        curr_msg = message[len_sent : min(len_sent+BUFFERSIZE, msg_len)]
        client.sendto(curr_msg, ADDR)
        len_sent += len(curr_msg)

def getfile(conn, filename):
    flag, addr = conn.recvfrom(FLAGSIZE)
    print(f"Getting file from {addr}")
    flag = int(flag.decode(FORMAT))
    dtime = 0
    fsize = 0
    if flag > 0:
        filesize, addr = conn.recvfrom(HEADERSIZE)
        filesize = int(filesize.decode(FORMAT))

        print(f"Size of requested file {filename} is: {filesize}")
        fname = CLIENTLIB + "protocol_udp_" + f"{BUFFERSIZE}_" + str(os.getpid()) + "_"+ filename 
        file = open(fname, "wb")
        len_recv = 0
        print(f"Writing to {fname}")
        start_time = time.time()
        while len_recv < filesize+FLAGITER*BUFFERSIZE:
            curr_msg, addr = conn.recvfrom(BUFFERSIZE)
            fsize+=len(curr_msg)
            if curr_msg[:len(ENDFLAG)] == ENDFLAG.encode(FORMAT):
                break
            try:
                file.write(curr_msg)
            except:
    ...
    fileThroughputDict[filename] = fsize/(dtime*1e6)
    return fsize, dtime
...
def request_file(filename, client):
    send_msg(filename, client)
    return filename
...
def run_client():
    ...
    indiv_time_log = f'udp_indiv_time_mode_{mode}.csv'
    indiv_throughput_log = f'udp_indiv_throughput_mode_{mode}.csv'
    agg_log = f'udp_agg_log_mode_{mode}.csv'
